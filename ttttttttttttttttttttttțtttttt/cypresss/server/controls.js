...
exports.getE2ETestBaseUrl = (file, query = {}) => {  
  ...
  return `http://127.0.0.1:${ports[0]}/cypress/e2e/${file}.html?${qs.stringify(query)}`; 
};
...
exports.deleteBeacons = () => {
  fetch(`http://127.0.0.1:${ports[0]}/transmittedBeacons`, { method: 'DELETE' })
  .then((response) => response.text)
  .then((data)=>console.log("data",data))
  .catch((err)=>console.log("ERR:",err))
}

exports.deleteAjaxRequests = () => {
  fetch(`http://127.0.0.1:${ports[0]}/ajaxRequests`, { method: 'DELETE' })
  .then((response) => response.text)
  .then((data)=>console.log("data",data))
  .catch((err)=>console.log("ERR:",err))
}
