...
app.delete('/ajaxRequests', (req, res) => {
  ajaxRequests = [];
  res.send('OK');
});
...
const beaconServerPorts = process.env.BEACON_SERVER_PORTS;

process.env.BEACON_SERVER_PORTS.split(',')   // split() splits a string into an array of substrings, and returns the array:
  .map(v => parseInt(v, 10))
  .forEach(port =>
    app.listen(port, () => {
